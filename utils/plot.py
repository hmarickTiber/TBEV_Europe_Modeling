...
def plot_xgboost_maps(df, cntry_codes, region='n', dot_sample=40000):
    ...
    ##################################################################################################
    df_foci = tick_df
    ## map tick species names as above and categorize
    df_foci['animal_category'] = np.where(df_foci['tick_animal'].str.lower().str.contains('tick'), df_foci['tick_animal'], 'Rodent reservoir')

    print(df_foci.animal_category.value_counts())
    gdf = gpd.GeoDataFrame(
        df_foci, geometry=gpd.points_from_xy(df_foci['longitude'], df_foci['latitude']))
    ...
    for _, r in gdf[gdf['tick_animal']=='Animal'].iterrows():
        ...
        folium.Polygon(locations=triangle, 
                    ...
                    fill_color=presence_colors[r['animal_category']], 
                    fill_opacity=fill_opac, 
                    color=presence_outlines[r['animal_category']],
                    opacity=opac,
                    weight=line_wt
                    ).add_to(mfa)
    mfa.add_to(m)
    ...
    <div style="position: fixed; 
        ...
        <div style="display: flex; width: 100%; align-items: center; margin-top: 10px;">
            <div style="margin-left: 5px;font-size:12px;color:#8202A9">&#9650;</div>
            <span style="font-size:10px; margin-left: 7px;">Rodent reservoir</span>
        </div>
    </div>
    '''
    ...
    binning = [0,1,5,10,15,25,100]
    ### Add in custom choropleth legend
    # Define your custom color palette as a LinearColormap
    #https://github.com/python-visualization/folium/issues/403
    custom_palette = cmp.StepColormap(
        ['#f4f4f4','#D0F0D4','#fde725','#5ec962','#21918c','#3b528b'],#,'#440154'], #viridis w less opaque
